...
export default class Surfer extends Phaser.Physics.Matter.Sprite {
    constructor(scene, x, y) {
        ...
        if (window.DeviceOrientationEvent && typeof DeviceOrientationEvent.requestPermission == 'function') {
            this.clickEvent = this.scene.input.once("pointerup", () => {
                alert('Device orientation avalable! If you\'d like to play with tilt controls, please allow the prompt...')
                // handle tilting of device;
                DeviceOrientationEvent.requestPermission().then((response) => {
                    if (response.granted) window.addEventListener("deviceorientation", (event) => {
